...
For example, changing the go package version from `v2` to `v3` bumps the import from `github.com/cosmos/ibc-go/v2` to `github.com/treasurenetprotocol/treasurenet_ibc`.
...
Attempting to import two versions of ibc-go, such as `github.com/cosmos/ibc-go/v2` and `github.com/treasurenetprotocol/treasurenet_ibc`, will result in multiple issues. 
...
`github.com/treasurenetprotocol/treasurenet_ibc` cannot be imported with any previous ibc-go versions
